...
export const ColorPicker: FunctionComponent<{
  ...
  onDraw?: () => void
}> = ({ color, size, onDraw, setColor }) => {
  const [canvasInfo, setCanvasInfo] = React.useState<{
    ctx: null | Expo2DContext
    width: number
    height: number
  ...
  React.useEffect(() => {
    ...
    const horizontalGradient = canvasInfo.ctx.createLinearGradient(
      0,
      0,
      scaledWidth,
      0
    ...
    const verticalGradient = canvasInfo.ctx.createLinearGradient(
      0,
      0,
      0,
      scaledHeight
